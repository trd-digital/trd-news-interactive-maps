...
const centerMiami = [-97.8423463448955,30.304976439140894]; // Miami's longitude and latitude
            ...
            .then(geojsonData => {
                ...
                map.on('click', 'unclustered-point', (e) => {
                    ...
                    if (properties['Address']) {
                        popupContent += `<h3>${properties['Address']}</h3>`;
                    }

                    // // Add the details (only the value)
                    // if (properties['Price']) {
                    //     popupContent += `<div class="popup-field">${properties['Price']}</div>`;
                    // }
                
                    // Add the status (both key and value)
                    if (properties['Price']) {
                        popupContent += `<div class="popup-field"><span class="popup-key">Price: </span><span class="popup-value">${properties['Price']}</span></div>`;
                    }

                    // Add the status (both key and value)
                    if (properties['Price psf']) {
                        popupContent += `<div class="popup-field"><span class="popup-key">Psf: </span><span class="popup-value">${properties['Price psf']}</span></div>`;
                    }

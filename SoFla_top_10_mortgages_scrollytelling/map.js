  ...
  map.addLayer(
    {
      ...
      source: {
        type: "geojson",
        data: "data/SoFla_top_mortgages.geojson",
      },
      paint: {
        "circle-radius": [
          'interpolate',
...
// Create the popup
map.on('click', 'interactive_layer', function (e) {
    let Amount = e.features[0].properties.Amount;
    let Project = e.features[0].properties.Project;
    let developers_borrowers = e.features[0].properties.Developers_borrowers;
    let Lenders = e.features[0].properties.Lenders;
    ...
    new mapboxgl.Popup()
        .setLngLat(e.lngLat)
        .setHTML('<h2>' + Project + '</h2>' 
        + '<strong>Amount:</strong> ' + Amount + '<br>'
        + '<strong>Developers/borrowers:</strong> ' + description_link + '<br>'
        + '<strong>Lenders:</strong> ' + Lenders + '<br>'
        + '<strong>Address:</strong> ' + Address + '<br>'
        + '<strong>County:</strong> ' + County + '<br>'
        .addTo(map));
});
// Change the cursor to a pointer when the mouse is over the defined layer.
map.on('mouseenter', 'interactive_layer', function () {
    map.getCanvas().style.cursor = 'pointer';
});
// Change it back to a pointer when it leaves.
map.on('mouseleave', 'interactive_layer', function () {

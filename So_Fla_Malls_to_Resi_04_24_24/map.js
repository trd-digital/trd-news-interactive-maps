  ...
  map.addLayer(
    {
      ...
      source: {
        type: "geojson",
        data: "data/SoFlaMallsToResi.geojson",
      },
      paint: {
        "circle-radius": [
          'interpolate',
  ...
  scroller
    ...
    .onStepEnter((response) => {
      ...
      thisLocation = {
        ...
        zoom: thisZoom,
        speed: 0.3, // Adjust this value as needed to change the transition speed
...
map.on('click', 'scrolly_layer', function (e) {
  ...
  let Description = e.features[0].properties.description_link;
  new mapboxgl.Popup({maxWidth:'285px'})
      ...
      '<hr style="border-top: 1px solid #ccc;">').addTo(map);
});
// Change the cursor to a pointer when the mouse is over the NAME layer.
map.on('mouseenter', 'scrolly_layer', function () {
  map.getCanvas().style.cursor = 'pointer';
});
// Change it back to a pointer when it leaves.
map.on('mouseleave', 'scrolly_layer', function () {
